...
| `src/App.jsx` | Route table (8 routes) + `RequireAuth` guard (**temporarily a pass-through while Login/Register are stubs** — restore the `isAuthed()` redirect when the real Login lands, keeping `/track` + `/tracking/:orderId` public) + `AppHeader` on every route. Routes in one place = a map of the whole app |
...
| Component | What / why |
|---|---|
| `AppHeader` | Top bar with **two variants**: guest (brand + Log in — public pages) vs authed (brand / Dashboard / Orders / New button / username / logout). Rendered on every route; the token in the auth store picks the variant |
...
11. **Why can guests see tracking but never order detail?** Two kinds of data with two
    sensitivity levels: progress (status/courier position) is what a tracking NUMBER
    entitles you to — the FedEx model, number-as-credential; detail (full addresses,
    receipt/review actions) is PII + mutation, so `/order/:id` stays behind login and
    ownership. A delivered order's tracking page also stays viewable — polling just
    stops at terminal states (DELIVERED/CANCELLED). The reference repo WeDelivery
    (Leluth) plays the same game one notch harder: its `/getorder` is anonymous but
    takes an **obfuscated token** (digit-shift + linear confusion + Base64 + random
    noise, see its TokenUtils) instead of a raw id, so order ids can't be enumerated;
    our contract keeps the raw orderId and relies on backend rate-limiting instead
