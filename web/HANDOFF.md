...
| **Yuning Zhang** | `pages/Tracking.jsx`, `pages/GuestTrack.jsx` | `/tracking/:orderId`, `/track` (public) | `api/tracking.js`: `getTracking` (poll every 5s) | Live position rendered on the map (the contract has current position only, no route history — if you need a trail, file a backend request). **Demo-stage spec**: StatusBadge + StatusTimeline + MapView courier dot; poll ONLY while PENDING/IN_TRANSIT, STOP at DELIVERED/CANCELLED (terminal pages stay viewable); guests see a "Log in to manage this order" hint — never link guests to `/order/:id` (PII + receipt/review actions). GuestTrack: order-number form (no login) that navigates to `/tracking/:orderId`; both routes must stay outside the auth guard |
