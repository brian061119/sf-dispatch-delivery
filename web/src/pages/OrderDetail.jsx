...
// Owner: Y (订单-详情 + 签收 + 评价). The live map stays on Tracking (Yuning);
// this page shows the static detail + post-delivery actions and links over.
...
export default function OrderDetailPage() {
    ...
    return (<Space orientation="vertical" style={{ width: '100%' }} size={16}>
      <Card title={<>
        ...
        {/* 签收 — PATCH /api/orders/:orderId/confirm-receipt */}
        <Button type="primary" disabled={signed || order.status === 'CANCELLED'} onClick={async () => {
            await confirmReceipt(orderId);
            message.success('Signed');
            load();
        }}>
          {signed ? 'Signed ✓' : 'Confirm receipt (签收)'}
        </Button>
      </Card>

      {/* 评价 + 损坏报备 — POST /api/orders/:orderId/review */}
      <Card title="Rate this delivery (评价)">
        <Form layout="vertical" onFinish={async (v) => {
          ...
          <Form.Item name="damageReported" label="Package damaged? (损坏报备)" valuePropName="checked">
            <Switch />
          </Form.Item>
